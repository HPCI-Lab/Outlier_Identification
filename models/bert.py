...
class BertFinetunePipeline(nn.Module): 
    ...
    def __init__(self, configs : RunConfig):
        ...
        self.model = BertModel.from_pretrained(configs.model.model_path, device_map=self.device, cache_dir=CACHE_PATH)
        self.task = "seq2seq"
        self.perturbate_embeddings = configs.model.perturbate_embeddings

        if configs.dataset.type in ["imdb"]: 
            self.task = "cls"
            self.lm_head = nn.Linear(768, 3).to(self.device)
        else: 
